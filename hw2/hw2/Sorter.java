...
public abstract class Sorter
{
	 ...
	 * 	if directory of PrintWriter does not exist
	 */
			...
			throws NullPointerException, IllegalArgumentException, FileNotFoundException
	{
		...
		String[] s = Arrays.copyOf(toSort.getArray(), Math.min(totalToSort, toSort.length()));
		WordList list = null;
		CountingComparator<String> cc = new CountingComparator<>(comp);
		...
		while (totalWordsSorted < totalToSort)
		{
			list = new WordList(s);
			sort(list, cc);
			totalWordsSorted += list.length();
		}
		totalSortingTime = System.currentTimeMillis() - timer;
		// end sorting timer
